...
description: 前端工程规范技能,提供Vue 3 + TypeScript + Vite项目的完整工程化规范指导,涵盖目录结构、代码规范、文件命名、路由、状态管理、组件封装、性能优化等16个核心维度。适用于所有前端开发任务,确保代码质量和团队协作一致性。
metadata:
  author: AI Assistant
  version: "1.0.0"
...
- **类型安全**: 必须使用 TypeScript,确保类型安全和开发体验
...
**Vue 组件：** 使用 Composition API + `<script setup lang="ts">`,遵循 script → template → style 顺序

**TypeScript：** 为所有变量添加类型,优先使用 interface,避免使用 any

**文件命名：** 组件用 PascalCase,TS 文件用 camelCase,目录用 kebab-case
...
- **代码质量**：命名规范、TypeScript 类型、ESLint/Prettier 检查
